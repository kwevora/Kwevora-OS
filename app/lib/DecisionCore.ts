...
function recallRelevantMemory(
  context: DecisionRequest,
): MemorySearchResult {
  const tags =
    buildMemoryTags(context);
  ...
  return [
    {
      ...
      label:
        "KAI assessed the current business context",
    ...
    },
    {
      ...
      label:
        "KAI identified the strongest current business risk",
...
function enrichDecisionContext(
  ...
  return {
    ...
    offers:
      context.offers ??
      context.businessProfile?.services,

    targetAudience:
      context.targetAudience ??
      context.businessProfile?.targetAudience,
    ...
    currentGoals:
      context.currentGoals ??
      context.businessProfile?.currentPriorities,

    connectedPlatforms:
      context.connectedPlatforms ??
      context.businessProfile?.platforms,
...
function buildRememberedKnowledge(
  memories: Memory[],
): string[] {
  return memories
    .map(
...
function buildAssumptions(
  ...
  if (
    context.revenueNeeded ||
    context.recentSales === 0
  ) {
    assumptions.push(
...
function buildPredictions(
  ...
  if (
    context.publishingReady &&
    context.publishingReady > 0
  ) {
    predictions.push(
...
function finalizeCognitiveSession(
  ...
  return {
    ...
    priority:
      decision.topOpportunity.title,

    decision:
      decision.recommendation,

    executionPlan: [
      ...decision.changeToday,
      ...decision.prepareNext,
    ],

    reflection:
      `After execution, KAI should compare the expected outcome with the actual result, record what worked, record what failed, and use that lesson to improve the next decision.`,
...
function saveDecisionMemory(
  ...
  const decisionMemoryId =
    `decision-${createdAt.replace(
      /[:.]/g,
      "-",
    )}`;
...
export class DecisionCore {
  think(
    ...
    const initialCognitiveSession =
      cognitiveCore.think({
        observation:
          buildCognitiveObservation(
            context,
            businessAssessment,
          ),

        rememberedKnowledge:
          buildRememberedKnowledge(
