...
async function ensurePublishingFile() {
  ...
  try {
    await fs.access(publishingFile);
  } catch {
    await fs.writeFile(
      publishingFile,
      "[]",
...
async function readPublishingQueue(): Promise<
  PublishingItem[]
> {
...
function cleanVideoPlan(
  ...
  const plan = value as Record<
    string,
    unknown
  >;

  return {
    openingText: cleanString(
      plan.openingText
    ),
...
function createPublishingItem(
  ...
  return {
    id:
      ...
      crypto.randomUUID(),
    createdAt:
      cleanString(value.createdAt) ||
      now,
    approvedAt:
      cleanString(value.approvedAt) ||
      now,
    updatedAt: now,
...
export async function POST(
  ...
  try {
    ...
    if (action === "add") {
      ...
      const newItem =
        createPublishingItem(sourceItem);
    ...
    if (action === "schedule") {
      ...
      const scheduledFor = cleanString(
        body.scheduledFor
      );
