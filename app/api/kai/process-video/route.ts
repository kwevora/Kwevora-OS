...
async function saveReviewQueue(items: ReviewItem[]) {
...
async function transcribeVideo(
  ...
  formData.append(
    "model",
    process.env.KAI_TRANSCRIPTION_MODEL ||
  ...
  if (!response.ok) {
    throw new Error(
      data?.error?.message ||
        "KAI could not transcribe the video."
    );
  }
...
function cleanIntelligenceReport(
  ...
  return {
    coreMessage: cleanText(report.coreMessage),
    contentGoal: cleanText(report.contentGoal),
    targetAudience: cleanText(report.targetAudience),
    audienceProblem: cleanText(report.audienceProblem),
    audienceDesire: cleanText(report.audienceDesire),
    primaryEmotion: cleanText(report.primaryEmotion),
    emotionalJourney: cleanText(report.emotionalJourney),
    strongestMoment: cleanText(report.strongestMoment),
    scrollStoppingReason: cleanText(
      report.scrollStoppingReason
    ),
    creatorVoice: cleanText(report.creatorVoice),
    recommendedAngle: cleanText(report.recommendedAngle),
    reasoningSummary: cleanText(report.reasoningSummary),
  };
}
...
async function analyzeTranscript(
    ...
    "https://api.openai.com/v1/responses",
    {
      ...
      body: JSON.stringify({
        ...
        text: {
          format: {
            type: "json_schema",
            name: "kwevora_video_intelligence_package",
            strict: true,
            schema: {
              type: "object",
  ...
  let analysis: KaiContentAnalysis;

  try {
    analysis = JSON.parse(outputText) as KaiContentAnalysis;
  } catch {
    throw new Error(
  ...
  return {
    intelligenceReport: cleanIntelligenceReport(
      analysis.intelligenceReport
    ),
    title: cleanText(analysis.title),
    hook: cleanText(analysis.hook),
    caption: cleanText(analysis.caption),
    hashtags: cleanHashtags(analysis.hashtags),
    thumbnailIdea: cleanText(analysis.thumbnailIdea),
    callToAction: cleanText(analysis.callToAction),
    platformRecommendation: cleanText(
      analysis.platformRecommendation
    ),
    publishingRecommendation: cleanText(
  ...
  try {
    ...
    if (!requestedId) {
      return NextResponse.json(
        {
          success: false,
          message: "A Review Queue item ID is required.",
        },
        { status: 400 }
      );
    }
    ...
    const itemIndex = reviewQueue.findIndex(
    ...
    if (itemIndex === -1) {
      return NextResponse.json(
        {
          success: false,
          message: "The Review Queue item was not found.",
        },
        { status: 404 }
      );
    }

    const currentItem = reviewQueue[itemIndex];

    if (!currentItem.media?.storedFileName) {
      return NextResponse.json(
        {
          success: false,
          message:
    ...
    const absoluteFilePath = path.join(
      uploadsFolder,
      currentItem.media.storedFileName
    );
    ...
    const latestQueue = await readReviewQueue();
    ...
    const processedItem: ReviewItem = {
      ...latestQueue[latestIndex],
      ...
      script: transcript,
      title:
        analysis.title || latestQueue[latestIndex].title,
      hook:
        analysis.hook || latestQueue[latestIndex].hook,
      caption:
        analysis.caption ||
        latestQueue[latestIndex].caption,
      hashtags:
        analysis.hashtags.length > 0
          ? analysis.hashtags
          : ["#KWEVORA"],
      thumbnailIdea: analysis.thumbnailIdea,
      callToAction: analysis.callToAction,
      platformRecommendation:
        analysis.platformRecommendation,
      publishingRecommendation:
        analysis.publishingRecommendation,
      intelligenceReport: analysis.intelligenceReport,
      idea:
        analysis.intelligenceReport.recommendedAngle ||
        analysis.intelligenceReport.coreMessage ||
        "KAI transcribed, understood, and prepared this video for publishing.",
    };

    latestQueue[latestIndex] = processedItem;
    await saveReviewQueue(latestQueue);
    ...
    console.error("KAI video processing failed:", error);
