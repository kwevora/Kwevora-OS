...
function normalizeCandidate(value: any): VerifiedCandidate {
  ...
  let verificationStatus: VerifiedCandidate["verificationStatus"] = [
    ...
    "unverified",
    "rejected",
  ].includes(value?.verificationStatus)
    ? value.verificationStatus
    : "unverified";

  const unresolvedFacts = cleanStringArray(value?.unresolvedFacts, 25);

  const sourceVerified = source?.verified === true;
  const licenseVerified = license?.verified === true;
  const resaleAllowed = license?.resaleToEndCustomers === true;
  const acquisitionVerified = acquisition?.verified === true;
  const pricingVerified = pricing?.verified === true;
  const demandVerified = demand?.verified === true;
  const facelessSuitable = facelessMarketing?.suitable === true;
  ...
  let readyForTesting = value?.readyForTesting === true && coreGatePassed;
  ...
  return {
    productName: cleanString(value?.productName) || "Unknown digital product",

    brand:
      cleanString(value?.brand) || cleanString(source?.seller) || "Unknown",
    ...
    license: {
      ...
      rebrandingAllowed: license?.rebrandingAllowed === true,
      modificationAllowed: license?.modificationAllowed === true,
      transferableResaleRights: license?.transferableResaleRights === true,
    ...
    acquisition: {
      ...
      recurringFees: cleanString(acquisition?.recurringFees),
    },

    resale: {
      verified: resale?.verified === true,
      permittedPrice: cleanString(resale?.permittedPrice),
      minimumPrice: cleanString(resale?.minimumPrice),
      suggestedPrice: cleanString(resale?.suggestedPrice),
    },
    ...
    quality: {
      ...
      freshnessConcerns: cleanString(quality?.freshnessConcerns),
    },

    competition: {
      verified: competition?.verified === true,
      score: clampScore(competition?.score),
      saturationRisk: cleanString(competition?.saturationRisk),
      details: cleanString(competition?.details),
    },

    restrictions: cleanStringArray(value?.restrictions, 20),
    ...
    requirements: {
      ...
      geographicRestrictions: cleanString(requirements?.geographicRestrictions),
    ...
    facelessMarketing: {
      ...
      legalAssetNotes: cleanString(facelessMarketing?.legalAssetNotes),
      contentAngles: cleanStringArray(facelessMarketing?.contentAngles, 10),
    },

    monetizationConfidence: clampScore(value?.monetizationConfidence),

    verifiedFacts: cleanStringArray(value?.verifiedFacts, 25),
    ...
    program: {
      ...
      currentlyAvailable: sourceVerified && acquisitionVerified,
    ...
    commission: {
      ...
      recurring: false,
      recurringDetails: cleanString(acquisition?.recurringFees),
    },
...
function buildCandidateText(candidates: CandidateInput[]): string {
  return candidates
    .map((candidate: CandidateInput, index: number) => {
      const unresolved = cleanStringArray(candidate.unresolvedFacts, 20);

      return [
        `CANDIDATE ${index + 1}`,
        `Product: ${cleanString(candidate.productName) || "Unknown"}`,
        `Seller / Brand: ${
        ...
        `Known license URL: ${cleanString(candidate.licenseUrl) || "Unknown"}`,
        `Known acquisition cost: ${
          cleanString(candidate.acquisitionCost) || "Unknown"
        }`,
        `Known resale price: ${
          cleanString(candidate.resalePrice) || "Unknown"
        }`,
        `Description: ${cleanString(candidate.whatItIs) || "None"}`,
        "",
        "FACTS STILL NEEDING VERIFICATION:",
        unresolved.length > 0
          ? unresolved.map((fact: string) => `- ${fact}`).join("\n")
...
async function deeplyVerify({
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      text: {
        format: {
          type: "json_schema",
          name: "kwevora_resellable_digital_product_verification",
          strict: true,
          ...
          schema: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    acquisition: {
                      ...
                      required: ["verified", "cost", "recurringFees"],
                    },
                    ...
                    margin: {
                      ...
                      required: ["verified", "details", "score"],
                    },
                    ...
                    demand: {
                      ...
                      required: ["verified", "score", "evidence", "details"],
                    },
            ...
            required: ["summary", "strongestVerifiedCandidate", "candidates"],
          },
        },
      },
    }),
   ...
   */
  const verifiedCandidates: VerifiedCandidate[] = Array.isArray(
    parsed?.candidates,
  )
    ? parsed.candidates.map((candidate: unknown) =>
        normalizeCandidate(candidate),
      )
    : [];

  const actuallyReady: VerifiedCandidate[] = verifiedCandidates
    .filter(
    ...
    .sort(
      (a: VerifiedCandidate, b: VerifiedCandidate) =>
        b.monetizationConfidence - a.monetizationConfidence,
    );

  const strongestVerifiedCandidate =
    actuallyReady.length > 0 ? actuallyReady[0].productName : "";
  ...
  try {
    ...
    if (!apiKey) {
      return NextResponse.json(
        {
          success: false,
          message: "OPENAI_API_KEY is missing from the KWEVORA environment.",
        },
        {
          status: 500,
        },
    ...
    const verificationRound = cleanRound(body.verificationRound);

    const focus = cleanString(body.focus);

    const candidates: CandidateInput[] = Array.isArray(body.candidates)
      ? body.candidates
          .filter((candidate: CandidateInput) =>
            Boolean(
    ...
    if (candidates.length === 0) {
      return NextResponse.json(
        {
          success: false,
          message: "KAI needs at least one digital-product finalist to verify.",
        },
        {
          status: 400,
        },
    ...
    const result = await deeplyVerify({
      apiKey,
      candidates,
      verificationRound,
      focus,
    ...
    await verifiedBusinessLaunchEngine.ingestVerification(
      result.report,
      result.sources,
    );
