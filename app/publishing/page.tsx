...
export default function PublishingPage() {
  ...
  async function loadPublishingQueue() {
    try {
      ...
      if (!response.ok) {
        throw new Error("KWEVORA could not load the Publishing Queue.");
      }

      const data = (await response.json()) as PublishingResponse;
      const loadedItems = Array.isArray(data.items) ? data.items : [];

      setItems(loadedItems);

      const initialScheduleValues: Record<string, string> = {};

      loadedItems.forEach((item) => {
        if (item.scheduledFor) {
          initialScheduleValues[item.id] = toDateTimeLocal(
            item.scheduledFor
          );
        }
      });
  ...
  async function publishToYouTube(item: PublishingItem) {
    if (!item.media) {
      setMessage(
    ...
    try {
      ...
      const response = await fetch("/api/youtube/upload", {
        ...
        body: JSON.stringify({
          title: item.title,
          description,
          tags: item.hashtags.map((hashtag) =>
            hashtag.replace(/^#/, "")
          ),
      ...
      const publishedResponse = await fetch("/api/publishing", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!publishedResponse.ok || !publishedData.success) {
        throw new Error(
          publishedData.message ||
            "The video reached YouTube, but KWEVORA could not update its publishing status."
        );
      }

      replaceItem(publishedData.item);
      ...
      setMessage(
        videoUrl
          ? `Video uploaded privately to YouTube: ${videoUrl}`
          : "Video uploaded privately to YouTube and marked as published."
      );
    } catch (error) {
  ...
  async function scheduleItem(item: PublishingItem) {
    const localDateTime = scheduleValues[item.id];
    ...
    if (Number.isNaN(scheduledDate.getTime())) {
      setMessage("Choose a valid publishing date and time.");
      return;
    }
    ...
    try {
      ...
      setMessage("Content package scheduled.");
    } catch (error) {
  ...
  async function markPublished(item: PublishingItem) {
    setWorkingId(item.id);
    setMessage("");
    ...
    try {
      ...
      replaceItem(data.item);
      setMessage("Content package marked as published.");
    } catch (error) {
  ...
  async function removeItem(item: PublishingItem) {
    ...
    try {
      ...
      setMessage("Content package removed from the Publishing Queue.");
    } catch (error) {
      console.error("Publishing Queue removal failed:", error);
  ...
  function replaceItem(updatedItem: PublishingItem) {
    setItems((currentItems) =>
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <section className="rounded-3xl border border-cyan-500/30 bg-cyan-500/10 p-8">
          ...
          <p className="mt-4 max-w-3xl text-lg leading-8 text-gray-300">
            Schedule, publish, and track every content package you have
            approved.
          </p>
        </section>
        ...
        {!loading && items.length === 0 ? (
          <section className="rounded-2xl border border-white/10 bg-white/5 p-8">
            ...
            <p className="mt-3 text-gray-300">
              Approve a content package in the Review Queue and KAI will
              move it here.
            </p>
          </section>
        ) : null}
                ...
                <div className="mt-8 grid gap-8">
                  ...
                  {item.status !== "published" ? (
                    <section className="rounded-2xl border border-amber-400/20 bg-amber-400/5 p-6">
                      <SectionHeading
                        eyebrow="Schedule"
                        title="Choose when this should go live"
                      />
                      ...
                      <div className="mt-6 flex flex-wrap items-end gap-3">
                        <label className="min-w-[280px] flex-1">
                          ...
                          <input
                            ...
                            className="mt-3 w-full rounded-xl border border-white/15 bg-black/30 px-4 py-3 text-white outline-none transition focus:border-amber-400/60"
                          />
                        </label>
                ...
                <div className="mt-8 flex flex-wrap gap-3 border-t border-white/10 pt-6">
                  {item.status !== "published" ? (
                    <>
                      <button
                        type="button"
                        onClick={() => publishToYouTube(item)}
                        disabled={isWorking || !item.media}
...
function StatusBadge({
  ...
  return (
    <div
      className={`rounded-full border px-4 py-2 text-sm font-bold ${styles[status]}`}
    >
      {statusLabel(status)}
    </div>
  );
...
function statusLabel(status: PublishingStatus) {
  switch (status) {
    case "approved":
      return "Approved";
